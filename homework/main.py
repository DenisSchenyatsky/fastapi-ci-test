...
async def recipes() -> List[models.Recipe]:
    ...
    """
    res = await session.execute(select(models.Recipe).order_by(desc(models.Recipe.watched_count), models.Recipe.name))
...
async def recipes(recipe_id: int = 1) -> models.Recipe:
    ...
    """
    res = await session.execute(select(models.Recipe).join(models.ProductRecipe).join(models.Product)
